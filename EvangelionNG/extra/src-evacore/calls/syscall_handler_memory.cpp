	...
	if (memory < CONST_KERNEL_AREA_START && (memory + pages * PAGE_SIZE) <= CONST_KERNEL_AREA_START) 
	{
		...
		if (targetThread) 
		{
			...
			if (virtualRangeBase != 0) 
			{
				...
				PageDirectory executing_space = AddressSpace::getCurrentSpace();

				// Map the pages to the other processes space
				for (uint32_t i = 0; i < pages; i++) 
				{
					PhysicalAddress physical_addr = AddressSpace::virtualToPhysical(memory + i * PAGE_SIZE);

					AddressSpace::switchToSpace(targetProcess->pageDirectory);
					AddressSpace::map(virtualRangeBase + i * PAGE_SIZE, physical_addr, DEFAULT_USER_TABLE_FLAGS, DEFAULT_USER_PAGE_FLAGS);
					AddressSpace::switchToSpace(executing_space);
				}
...
SYSCALL_HANDLER(unmap) 
{
	...
	if (range) 
	{
		...
		if (range->flags & PROC_VIRTUAL_RANGE_FLAG_PHYSICAL_OWNER) 
		{
			...
			{
				PhysicalAddress physicalPage = AddressSpace::virtualToPhysical(range->base + i * PAGE_SIZE);
				PPallocator::free(physicalPage);
			}
		}
...
SYSCALL_HANDLER(sbrk) 
{
	...
	if (process->heapBreak == 0) 
	{
		...
		AddressSpace::map(heapStart, phys, DEFAULT_USER_TABLE_FLAGS,
	...
	{
		// expand if necessary
		VirtualAddress virt_above;
		while (brkNew > (virt_above = process->heapStart + process->heapPages * PAGE_SIZE)) 
		{
			PhysicalAddress phys = PPallocator::allocate();
			AddressSpace::map(virt_above, phys, DEFAULT_USER_TABLE_FLAGS,
			DEFAULT_USER_PAGE_FLAGS);
			PPreferenceTracker::increment(phys);
			++process->heapPages;
		}
		...
		// shrink if possible
		VirtualAddress virt_below;
		while (brkNew < (virt_below = process->heapStart + process->heapPages * PAGE_SIZE - PAGE_SIZE)) 
		{
			PhysicalAddress phys = AddressSpace::virtualToPhysical(virt_below);
			AddressSpace::unmap(virt_below);
			if (PPreferenceTracker::decrement(phys) == 0) 
			{
