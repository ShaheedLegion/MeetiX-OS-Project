...
void FileSystem::findExisting(char *absolutePath, FsNode **outParent, FsNode **outChild, char *nameCurrent, bool followSymlinks) 
{
	...
	while (true) 
	{
		...
		*curpos = 0;
		...
		else if (String::equals(".", nameCurrent)) 
		{
			// skip
		} 
 ...
 *
 */
Fd FileSystem::mapFile(Pid pid, FsNode *node, int32_t openFlags, Fd fd) 
{
 ...
 *
 */
bool FileSystem::nodeForDescriptor(Pid pid, Fd fd, FsNode **outNode, FileDescriptorContent  **outFd) 
{
 ...
 *
 */
Fd FileSystem::clonefd(Fd sourceFd, Pid sourcePid, Fd targetFd, Pid targetPid, FsClonefdStatus *outStatus) 
{
	FsNode *sourceNode;
	...
	Fd created = mapFile(targetPid, sourceNode, 0, targetFd);
	if (created != -1) 
	{
		// clone fd contents
...
FsPipeStatus FileSystem::pipe(Thread *thread, Fd *outWrite, Fd *outRead) 
{
