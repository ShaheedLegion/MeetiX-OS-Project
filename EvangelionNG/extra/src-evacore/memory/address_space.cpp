 ...
 */
bool AddressSpace::map(VirtualAddress virtualAddr, PhysicalAddress physicalAddr, uint32_t tableFlags, uint32_t page_flags, bool allowOverride) 
{

	// check if addresses are aligned
	if ((virtualAddr & PAGE_ALIGN_MASK) || (physicalAddr & PAGE_ALIGN_MASK)) 
	{
		EvaKernel::panic("%! tried to map unaligned addresses: virt %h to phys %h", "addrspace", virtualAddr, physicalAddr);
	}
	...
	{
		table[pi] = physicalAddr | page_flags;
 ...
 *
 */
void AddressSpace::mapToTemporaryMappedDirectory(PageDirectory directory, VirtualAddress virtualAddr, PhysicalAddress physicalAddr, uint32_t tableFlags, uint32_t page_flags, bool allowOverride) 
{
