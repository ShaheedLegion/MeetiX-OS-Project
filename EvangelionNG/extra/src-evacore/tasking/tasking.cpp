 ...
 * 
 */
void Tasking::addTask(Thread *t, bool enforceCurrentCore) 
{
	Scheduler *target = 0;
...
uint32_t previousSchedCounts = 0;

/**
 *
 */
Thread *Tasking::getAtPosition(uint32_t position) 
{
	uint32_t processors = System::getNumberOfProcessors();
	for (uint32_t i = 0; i < processors; i++) 
	{
		...
		if (scheduler) 
		{
			...
			uint32_t tasksForSched = scheduler->count();
			
			if (position < tasksForSched) return scheduler->getAtPosition(position - previousSchedCounts);

			previousSchedCounts = previousSchedCounts + tasksForSched;
		}
	}
