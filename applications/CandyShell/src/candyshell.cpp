...
#include <mxuser/utils/power.hpp>
...
int main(int argc, char *argv[]) 
{
	...
	{	
		if (TaskGetID("CandyText") != -1) 
		{
			...
			FileLog::flog("error: CandyShell can only be executed once when in headless mode");
			return -1;
		}
	}
	...
	if (!headless) 
	{
		...
		{
			FileLog::flog("failed to open UI for CandyShell with status " + status);
			return -1;
		}
	}
...
void CandyShell::prepare() 
{
	...
	if (headless) 
	{
		...
		FileLog::flog("initializing headless CandyShell");
	...
	else FileLog::flog("initializing graphic CandyShell");

	// load keyboard layout
	string initialLayout = "it-EU";
	if (Keyboard::loadLayout(initialLayout)) FileLog::flog("keyboard layout '" + initialLayout + "' loaded");

	else 
	{
		FileLog::flog("unable to load keyboard layout '" + initialLayout + "'");
		return;
	}
}
...
void CandyShell::shellStartRoutine(CreateShellInfo *inf) 
{
	...
	threadName << "CandyShell/tty" << inf->number;
...
void CandyShell::run(CreateShellInfo *inf) 
{
	...
	if (headless) 
	{
		if (!inf->number) 
		{
			...
			if (logofile.good()) 
			{
				...
				screen->write(logo, color->color(SC_STD_BACK, SC_GREEN));
				logofile.close();
			}

			else screen->write("\t\t\t\tLogo file not found", color->errorColor());
			...
			screen->write(msg2.str(), color->defaultColor());
		} 
	...
	while (true) 
	{
		...
		if (writeHeader) 
		{		
			if (currentUser == "root") screen->write(hostname, color->color(SC_STD_BACK, SC_RED));

			else screen->write(hostname, color->color(SC_STD_BACK, SC_GREEN));
			
			screen->write("@", color->defaultColor());
			screen->write(currentUser, color->color(SC_STD_BACK, SC_GREEN));
			screen->write(":", color->defaultColor());
			screen->write(workingDirectory, color->color(SC_STD_BACK, SC_CYAN));
			screen->write(">", color->defaultColor());
			screen->updateCursor();
		} 
...
void CandyShell::runCommand(string command) 
{
	...
	errData.id = "CandyShell/stdErrOut";
	errData.stop = false;
	errData.err = true;
 ...
 *
 */
bool CandyShell::runTermCommand(string command, Fd *termIn, Fd *termOut, Fd *termErr, Pid *intPid) 
{
	// create initial pipe
	...
	{
		screen->write("unable to create process pipe", color->errorColor());
		return false;
	}
	*termIn = firstPipeW;
	Fd lastStdout = firstPipeR;
	...
	for (int i = 0; i < numparts; i++) 
	{
		...
		{
			screen->write("unable to create process out pipe", color->errorColor());
			return false;
		}
		...
		{ // stderr only for last
			...
			if (pipeStat != FS_PIPE_SUCCESSFUL) 
			{
				screen->write("unable to create process err pipe", color->errorColor());
				return false;
			}
			inStdio[2] = errPipeW;
			*termErr = errPipeR;
...
bool CandyShell::handleBuiltin(string command) 
{
	...
	{
		TimeDriverCall *time = MXtime::getTime();
		...
		screen->write(Time.str());
		screen->write(Date.str());
	...
	{
		screen->write("rebooting MeetiX OS");
		Power::sendReboot();
	...
	{
		screen->write("Shutting down MeetiX OS");
		Power::sendShutdown();
	...
	{
		TimeDriverUptime *uptime = MXtime::getUptime();
...
void CandyShell::standardOutThread(StandardOutThreadData *data) 
{
	...
	while (true) 
	{
		...
		{
			uint8_t localColor = (data->err ? color->errorColor() : color->defaultColor());
...
bool CandyShell::findExecutable(string name, string &out) 
{
	...
	string appPath = Environment::getEnvVar("PATH") + name;
	...
	// if file not exist in path user workingdirectory to search executable
	else if (fileExists(workingDirectory + name))
	{
		out = workingDirectory + name;
		return true;
	}
 ...
 *
 */
bool CandyShell::execute(string shortpath, string args, Pid *outPid, Fd outStdio[3], Fd inStdio[3]) 
{
	...
	screen->write(msg.str(), color->errorColor());
	return false;
}
...
string CandyShell::readInput(string there, Screen *screen, ShellInputStatus *outStatus, bool *continueInput, bool visible) 
{
	...
	{
		KeyInfo key = screen->readInput(continueInput);
		...
		if (key.pressed) 
		{
			...
			else if (key.key == "KEY_TAB") 
			{
				...
				while ((ent = readdir(dir)) != 0) 
				{
					if (searchlen < ent->d_namlen && strncmp(search.c_str(), ent->d_name, search.length()) == 0) 
					{
						...
						screen->write(appendix.str(), color->defaultColor());
						break;
					}
				}
			...
			// show history
			else if (key.shift && key.key == "KEY_TAB")
			{
				string output;
				...
				screen->write(line, color->defaultColor());
				...
				{
					output = output + chr;
					screen->write(output, color->defaultColor());
				}
			}
			...
			else
			{
				...
				if (chr != -1) 
				{
					...
					string out;
					out = out + chr;
					screen->write(out, color->defaultColor(), visible);
				}
			}
