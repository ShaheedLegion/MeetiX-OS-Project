...
void CandyTerminal::execute() 
{
	...
	if (!screen) 
	{
		FileLog::flog("CandyTerminal: Failed to initialize screen");
		return;
	}
	screen->clean();
	...
	{
		Logger::log("CandyTerminal: Failed to load keyboard layout: " + initialLayout);
		return;
	}
...
void CandyTerminal::initializeScreen() 
{
	...
	else FileLog::flog("CandyTerminal: Failed to initialize the graphical screen");
}
...
void CandyTerminal::startShell() 
{
	...
	{
		FileLog::flog("CandyTerminal: Failed to setup stdin pipe for shell");
		return;
	}
	...
	{
		FileLog::flog("CandyTerminal: Failed to setup stdout pipe for shell");
		return;
	}
	...
	{
		FileLog::flog("CandyTerminal: Failed to setup stderr pipe for shell");
		return;
	}
	...
	stdioIN[2] = shellerrW;
	Fd stdioTarget[3];
	SpawnStatus status = SpawnPOI("/bin/mx", "-sh", "/", SECURITY_LEVEL_APPLICATION, &outPid, stdioTarget, stdioIN);

	if (status != SPAWN_STATUS_SUCCESSFUL) 
	{
		FileLog::flog("CandyTerminal: Failed to spawn mx shell process");
		return;
	}
...
void CandyTerminal::inputRoutine() 
{
	...
	while (true) 
	{
		KeyInfo readInput = screen->readInput();
