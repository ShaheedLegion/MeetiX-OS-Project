...
void MXinterpreter::print(LsStatement *stat) 
{
	...
	FileLog::flog(stat->pairs[0]->value.c_str());
}
...
void MXinterpreter::wait(LsStatement *stat) 
{
	...
	while (TaskGetID(identifier.c_str()) == -1) 
	{
		Yield();
	}
}
...
/**
 *	exec application with passed security level
 */
Pid MXinterpreter::execWithSpawner(string path, string args, SecurityLevel secLvl) 
{
	Pid pid = -1;
	SpawnStatus stat = SpawnP(path.c_str(), args.c_str(), "/", secLvl, &pid);
	
	if (stat != SPAWN_STATUS_SUCCESSFUL) 
	{
		FileLog::flog("failed to spawn '" + path + "' with code");
	 	cout << "failed to spawn '"  << path  << "' with code" << endl;
	}
