...
int pciList() 
{
	...
	EvaQueryPciCountOut out;

	EvaQueryStatus countstatus = EvaQuery(EVAQUERY_PCI_COUNT, 0, (uint8_t*) &out);
	...
	for (uint32_t pos = 0; pos < deviceCount; pos++) 
	{
		...
		EvaQueryPciGetOut out;

		EvaQueryStatus getstatus = EvaQuery(EVAQUERY_PCI_GET, (uint8_t*) &in, (uint8_t*) &out);
		...
		printf("\xB3 %02x \xB3 %02x \xB3 %02x \xB3 %04x \xB3 %04x \xB3 %.22s \xB3 %.22s \xB3", out.bus, out.slot, out.function, out.vendorId, out.deviceId, className, subclassName);
	}
