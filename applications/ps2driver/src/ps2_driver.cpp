...
#include <mxuser/io/ps2_driver_constants.hpp>
...
int main() 
{
	...
	// register
	if (!TaskRegisterID(PS2_DRIVER_IDENTIFIER)) 
	{
		std::stringstream log;
		log << "failed to register as " << (char*) PS2_DRIVER_IDENTIFIER;
		FileLog::flog(log.str());
		return 1;
	}
	...
	if (sharedArea == 0) 
	{
		FileLog::flog("failed to allocate transfer memory area");
		return 1;
	}
...
void initializeMouse() 
{
	...
	// empty input buffer
	while (Utils::inportByte(0x64) & 0x01) 
	{
		Utils::inportByte(0x60);
	}

	// activate mouse device
	waitForBuffer(PS2_OUT);
	Utils::outportByte(0x64, 0xA8);

	// get commando-byte, set bit 1 (enables IRQ12), send back
	waitForBuffer(PS2_OUT);
	Utils::outportByte(0x64, 0x20);

	waitForBuffer(PS2_IN);
	status = Utils::inportByte(0x60) | (1 << 1);

	waitForBuffer(PS2_OUT);
	Utils::outportByte(0x64, 0x60);
	waitForBuffer(PS2_OUT);
	Utils::outportByte(0x60, status);

	// send set-default-settings command to mouse
	writeToMouse(0xF6);

	waitForBuffer(PS2_IN);
	status = Utils::inportByte(0x60);
	if (status != 0xFA) 
	{
		FileLog::flog("mouse did not ack set-default-settings command");
		return;
	}
	...
	if (status != 0xFA) 
	{
		FileLog::flog("mouse did not ack enable-mouse command");
		return;
	}
}
...
void writeToMouse(uint8_t value) 
{
	...
	waitForBuffer(PS2_OUT);
	Utils::outportByte(0x64, 0xD4);

	// send the value to the mouse
	waitForBuffer(PS2_OUT);
	Utils::outportByte(0x60, value);
}
