...
std::string rg1[] =
{
	"Ciao " + Environment::getEnvVar("USER") + " benvenuto in MeetiX OS " + Environment::getEnvVar("VERSION"),
	"Ok cominciamo",
	"",
	"Il terminale grafico CandyShell permette di interagire piu a fondo con il sistema,",
	"",
};
...
std::string rg2[] =
{
	"Ti guiderò con una breve intro ai principali aspetti dell'interfaccia grafica",
	"La CandyGui ha ora uno stile Gnome, quindi troverei nella barra in alto,",
...
std::string rg5[] =
{
	"http://meetixosproject.weebly.com - t.me/MeetiXOSProject",
...
/*
 *	method called when user click X button
 */
...
void showPage(int index)
{
	...
	else if (index == 5) closeApplication();
}
...
int main(int argc, char *argv[])
{
	...
	if (openStat == UI_OPEN_STATUS_SUCCESSFUL) 
	{
		...
		mainWindow->onClose(closeApplication);
