 ...
 *
 */
void EventProcessor_t::bufferKeyEvent(KeyInfo keyInfo) 
{
...
void EventProcessor_t::processCommand(Tid senderTid, UiMessageHeader *requestHeader, CommandMessageResponse_t &responseOut) 
{
	if (requestHeader->id == UI_PROTOCOL_CREATE_COMPONENT) 
	{
		...
		default:
			FileLog::flog("don't know how to create a component of type " + createRequest->type);
			break;
		}
		...
		if (component != 0) 
		{
			...
			component->setBounds(Rectangle(100, 100, 200, 80));
		}
	...
	else if (requestHeader->id == UI_PROTOCOL_REMOVE_COMPONENT)
	{
		...
		UiRemoveComponentResponse *response = new UiRemoveComponentResponse();
		
		if (ComponentRegistry_t::removeComponent(senderTid, deletionRequest->id)) response->status = UI_PROTOCOL_SUCCESS;
		else response->status = UI_PROTOCOL_FAIL;

		responseOut.message = response;
		responseOut.length = sizeof(UiRemoveComponentResponse);
	}
	...
	else if (requestHeader->id == UI_PROTOCOL_ADD_COMPONENT) 
	{
		...
		if (parent == 0 || child == 0) 
		{
			...
			std::stringstream ss;
			ss << "could not add " << request->child << "(" << child << ") to " << request->parent << "(" << parent << ")";
			FileLog::flog(ss.str());
		} 
	...
	else if (requestHeader->id == UI_PROTOCOL_SETUP_PNG)
	{
	...
	else if (requestHeader->id == UI_PROTOCOL_SETUP_COMPONENT_COLOR)
	{
	...
	else if (requestHeader->id == UI_PROTOCOL_REGISTER_TASK_MANAGER)
	{
		...
		else 
		{
			...
			if (label == nullptr) response->status = UI_PROTOCOL_FAIL;
 ...
 *
 */
void EventProcessor_t::translateKeyEvent(KeyInfo &info) 
{
