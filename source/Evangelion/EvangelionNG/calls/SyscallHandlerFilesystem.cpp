...
SYSCALL_HANDLER(getExecutablePath)
{
	...
	if (currentThread->process->sourcePath == nullptr) data->buffer[0] = 0;
...
SYSCALL_HANDLER(setWorkingDirectory)
{
	...
	if (data->process == 0) target = currentThread; // any task can do this for himself
...
SYSCALL_HANDLER(fsReadDirectory)
{
	...
	auto folder = FileSystem::getNodeById(data->iterator->nodeID);
	if (folder == nullptr)
	{
