...
SYSCALL_HANDLER(createPagesInSpace)
{
	...
	if (process->securityLevel == SECURITY_LEVEL_KERNEL)
	{
		...
		if (numberOfPages > 0 && numberOfPages <= CREATE_PAGE_IN_SPACE_MAXIMUM_PAGES)
		{
			...
			if (targetProcess->imageStart == 0 || targetProcess->imageStart > virtualAddressInTargetSpace) targetProcess->imageStart = virtualAddressInTargetSpace;
			if (targetProcess->imageEnd == 0 || targetProcess->imageEnd < (virtualAddressInTargetSpace + numberOfPages * PAGE_SIZE)) targetProcess->imageEnd = virtualAddressInTargetSpace + numberOfPages * PAGE_SIZE;
...
SYSCALL_HANDLER(ramdiskSpawn)
{
	...
	if (process->securityLevel == SECURITY_LEVEL_KERNEL)
	{
		...
		Elf32SpawnStatus status = Elf32Loader::spawnFromRamdisk(data->path, data->securityLevel, &spawnedThread, false, THREAD_PRIORITY_NORMAL);
		...
		else if (status == ELF32_SPAWN_STATUS_FILE_NOT_FOUND)
		{
