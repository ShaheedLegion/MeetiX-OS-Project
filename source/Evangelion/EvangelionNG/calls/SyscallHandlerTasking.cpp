...
SYSCALL_HANDLER(kill)
{
	...
	if (target == nullptr) data->status = KILL_STATUS_NOT_FOUND;
...
/**
 * Returns the identifier string of provided Tid
 */
SYSCALL_HANDLER(getIdentifier)
{
 ...
 *
 */
SYSCALL_HANDLER(taskIdRegister)
{
	TaskIDRegister *data = (TaskIDRegister*) SYSCALL_DATA(currentThread->cpuState);
	data->successful = Tasking::registerTaskForIdentifier(currentThread, data->identifier);
	return currentThread;
}
...
SYSCALL_HANDLER(fork)
{
	...
	if (forked)
	{
		...
		auto cur = AddressSpace::getCurrentSpace();
...
SYSCALL_HANDLER(raiseSignal)
{
	...
	if (data->signal >= 0 && data->signal < SIG_COUNT)
	{
		...
		if (targetThread == nullptr) data->status = RAISE_SIGNAL_STATUS_INVALID_TARGET;
