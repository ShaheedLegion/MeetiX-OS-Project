...
void IoApic::createMapping()
{
	...
	if (virtualAddress == 0) EvaKernel::panic("%! could not get a virtual range for mapping", "ioapic");
	...
	AddressSpace::map(PAGE_ALIGN_DOWN(virtualAddress) + PAGE_SIZE, PAGE_ALIGN_DOWN(physicalAddress) + PAGE_SIZE, DEFAULT_KERNEL_TABLE_FLAGS,
