...
void GuiScreen::paint()
{
	...
	while (true)
	{
		...
		for (PositionedGlyph &g : viewModel->positions)
		{
			...
			cairo_save(cr);
			cairo_set_source_rgba(cr, COLOR_ARGB_TO_FPARAMS(colors[i]));
 ...
 *
 */
void GuiScreen::writeChar(char c, ColorArgb color)
{
	if (charIsUtf8(c))
	{
		output = output + c;
 ...
 *
 */
void GuiScreen::write(string message, ColorArgb color, bool visible)
{
	for (char c : message)
		writeChar(c, color);
}
