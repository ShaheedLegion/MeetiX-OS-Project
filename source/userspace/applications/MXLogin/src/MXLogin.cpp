...
#include <mxuser/utils/fparser.hpp>
...
static void loginToMeetiX()
{
	...
	if (researchAccess(username, password, mode))
	{
		...
		if (mode == LOGIN)
		{
			...
			FileLog::flog("Logging as: %s", username.c_str());
...
static bool researchAccess(string username, string password, LoginMode_t mode)
{
	...
	for (pair<string, string> current : properties)
	{
		...
		{
			if (mode != LoginMode_t::LOCK) Environment::setEnvVar("USER", username);
			return true;
		}
	}
...
void configureActionComponents()
{
	...
	else if (mode == LOCK)
	{
		...
		usernameLabel->setTitle(Environment::getEnvVar("USER"));
	}
