...
#include <mxuser/ui/notification.hpp>
...
{
	// create a base path
	std::string basepath = "/app/zipNET/deps/background/";
...
static void okayHandler()
{
	if (UI::setBackground(*it))
		Notification::sendNotification("UItweak background changer", "Successful applied " + std::string(basename((char*)(*it).c_str())) + std::string(" as background"), 5, RGB(0, 200, 0), MessageAnimation_t::SIDE_SCROLL);

	else
		Notification::sendNotification("UItweak background changer", "Unable to apply " + std::string(basename((char*)(*it).c_str())) + std::string(" as background"), 5, RGB(200, 0, 0), MessageAnimation_t::DARKENING);
}
