...
void TextField_t::paint()
{
	...
	// background
	cairo_set_source_rgba(cr, COLOR_ARGB_TO_FPARAMS(sColor));
	...
	cairo_set_source_rgba(cr, COLOR_ARGB_TO_FPARAMS(borderColor));
	cairo_rectangle(cr, 0, 0, bounds.width, bounds.height);
	cairo_stroke(cr);
	...
	if (focused)
	{
		...
		{
			ColorArgb color = textColor;
			if (first != second && pos >= first && pos < second)
			{
				cairo_set_source_rgba(cr, COLOR_ARGB_TO_FPARAMS(ARGB(255, 0, 200, 0)));
	...
	for (PositionedGlyph &g : viewModel->positions)
	{
		...
		cairo_save(cr);
		cairo_set_source_rgba(cr, COLOR_ARGB_TO_FPARAMS(color));
	...
	if (focused)
	{
		...
		markFor(COMPONENT_REQUIREMENT_UPDATE);
		cairo_set_source_rgba(cr, COLOR_ARGB_TO_FPARAMS(ARGB(255, 60, 60, 60)));
 ...
 *
 */
void TextField_t::setColor(ColorArgb shapeColor, ColorArgb titleColor)
{
	sColor = shapeColor;
 ...
 *
 */
void TextField_t::backspace(KeyInfo &info)
{
	if (text.length() > 0)
	{
