...
void ZipNET::launch()
{
	...
	FileLog::flog("loading keyboard layout '%s'", keyLayout.c_str());
...
void lockCheck()
{
	while (true)
	{
		if (Millis() - renderStart > 6000) FileLog::flog("window server has frozen");
		Sleep(1000);
	}
}
...
void ZipNET::mainLoop(Rectangle screenBounds)
{
	...
	Environment::setEnvVar("SYSTEM_LEVEL", "interactive");
...
{
	if (!Cursor_t::load("/app/zipNET/deps/cursor/default.cursor")) FileLog::flog("Unable to load default cursor");
	if (!Cursor_t::load("/app/zipNET/deps/cursor/text.cursor")) FileLog::flog("Unable to load text cursor");
	if (!Cursor_t::load("/app/zipNET/deps/cursor/resize-ns.cursor")) FileLog::flog("Unable to load resize-ns cursor");
	if (!Cursor_t::load("/app/zipNET/deps/cursor/resize-ew.cursor")) FileLog::flog("Unable to load resize-ew cursor");
	if (!Cursor_t::load("/app/zipNET/deps/cursor/resize-nesw.cursor")) FileLog::flog("Unable to load resize-nesw cursor");
	if (!Cursor_t::load("/app/zipNET/deps/cursor/resize-nwes.cursor")) FileLog::flog("Unable to load resize-nwes cursor");
