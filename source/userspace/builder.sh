...
function setBuildTarget()
{
	...
	then
		ARTIFACT_TARGET=$MXfs"/cmd/"$ARTIFACT_NAME

	elif [[ $ARTIFACT_TYPE == "application" ]];
	then
		ARTIFACT_TARGET=$MXfs"/app/"$ARTIFACT_NAME"/bin/"$ARTIFACT_NAME

	elif [[ $ARTIFACT_TYPE == "server" ]];
	then
		ARTIFACT_TARGET=$MXfs"/sys/eva/server/"$ARTIFACT_NAME
	fi
}
...
{
	echo "[${RED}cleaning${RESET}] $ARTIFACT_TARGET"
...
function targetLink()
{
	...
	$COMPILER -o $ARTIFACT_LOCAL $OBJ/*.o -lmxuser $LDFLAGS
}
