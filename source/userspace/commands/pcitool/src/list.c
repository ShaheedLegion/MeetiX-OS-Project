...
int pciList()
{
	...
	EvaQueryPciCountData out;

	EvaQueryStatus countstatus = EvaQuery(EVAQUERY_PCI_COUNT, (uint8_t*) &out);
	...
	{
		EvaQueryPciGetData buf;
		buf.position = pos;

		EvaQueryStatus getstatus = EvaQuery(EVAQUERY_PCI_GET, (uint8_t*) &buf);
		...
		printf("\xB3 %02x \xB3 %02x \xB3 %02x \xB3 %04x \xB3 %04x \xB3 %.22s \xB3 %.22s \xB3", buf.bus, buf.slot, buf.function, buf.vendorId, buf.deviceId, className, subclassName);
	}
