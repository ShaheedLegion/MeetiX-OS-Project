...
#include <mxuser/io/ps2const.hpp>
...
{
	// register
	if (TaskGetID(PS2_DRIVER_IDENTIFIER) == -1)
	{
		FileLog::flog("Ps2Driver: failed to register as %s", PS2_DRIVER_IDENTIFIER);
		return 1;
	}
...
void waitForBuffer(Ps2Buffer_t buffer)
{
	...
	{
		while (timeout--)
			if ((Utils::inportByte(0x64) & 2) == 0) return;
	}

	else if (buffer == PS2_IN)
	{
		while (timeout--)
			if ((Utils::inportByte(0x64) & 1) == 1) return;
	}
}
