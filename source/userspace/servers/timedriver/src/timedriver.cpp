...
void Cmos_t::timer(Cmos_t *instance)
{
    // read the bios
    instance->readRTC();
    FileLog::flog("time: %i/%i/%i %i:%i:%i", var.day, var.month, var.year, var.hour + 2, var.minute, var.second);
...
{
    // ask to kernel if there are another time driver in execution
	if (TaskGetID(TIME_DRIVER_IDENTIFIER) == -1)
	{
	    FileLog::flog("TimeDriver: could not register with task identifier %s", TIME_DRIVER_IDENTIFIER);
		return -1;
	}
	...
	while (true)
	{
		...
		else FileLog::flog("uknown command: %i from task: %i", timeheader->command, header->sender);
	}
